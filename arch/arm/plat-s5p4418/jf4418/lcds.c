...
static int __init jf4418_setup_lcd(char *str)
{
	char *delim;
	int i;
...
__ret:
	board_set_ctp(jf4418_lcd_config[lcd_idx].ctp);

	printk("Display: %s selected\n", jf4418_lcd_config[lcd_idx].name);
	return 0;
}
early_param("lcd", jf4418_setup_lcd);
...
void jf4418_get_lcd_res(int *w, int *h)
{
	struct nxp_lcd *lcd = jf4418_lcd_config[lcd_idx].lcd;
...
EXPORT_SYMBOL(jf4418_get_lcd_res);
...
static int __init jf4418_init_ctp(char *str)
{
	unsigned int val;
	char *p = str, *end;
	...
	if (val < CTP_MAX && jf4418_lcd_config[lcd_idx].ctp) {
		ctp_type = val;
...
__setup("ctp=", jf4418_init_ctp);

unsigned int jf4418_get_ctp(void)
{
	if (jf4418_lcd_config[lcd_idx].ctp)
		return ctp_type;
	else
		return CTP_NONE;
}
EXPORT_SYMBOL(jf4418_get_ctp);

void jf4418_set_ctp(int type)
{
	if (ctp_type == CTP_AUTO && type < CTP_MAX) {
		ctp_type = type;
	}
}
EXPORT_SYMBOL(jf4418_set_ctp);
#endif
